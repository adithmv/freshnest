...
import { Home, Plus, X, Clock, ShoppingBag, Check, LogOut, Package, TrendingUp, Upload, Image, MapPin, Store, Camera } from "lucide-react";
...
export default function SellerDashboard() {
  ...
  async function submitListing(e) {
    ...
    if (imageFile) {
      ...
      if (imageUrl) await supabase.from("listings").update({ images: [imageUrl] }).eq("id", listing.id);
    }
  ...
  return (
    <div style={{ fontFamily: "DM Sans, sans-serif", background: "#fafafa", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "28px 24px" }}>
            ...
            return (
              <div key={s.label} className="stat-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                  <div style={{ width: 34, height: 34, background: "#f5f5f5", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Icon size={15} color="#444" strokeWidth={1.5} />
                  </div>
        ...
        {/* Tabs */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #f2f2f2", marginBottom: 28 }}>
          <div style={{ display: "flex", gap: 28 }}>
            {[["orders", "Orders"], ["listings", "My Listings"], ["shop", "Shop Profile"]].map(([key, label]) => (
                  ...
                  <div key={l.id} className="listing-card">
                    ...
                    <div style={{ padding: 16 }}>
                      ...
                      <div style={{ display: "flex", gap: 10, fontSize: 12, color: "#aaa", marginBottom: 10 }}>
                        <span>{Math.max(0, l.available_qty)} / {l.total_quantity} left</span>
                        <span>·</span>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6, background: u.bg, borderRadius: 7, padding: "6px 10px", marginBottom: 10 }}>
                        ...
                        <span style={{ fontSize: 11, color: u.color, fontWeight: 500 }}>
                          {u.expired ? "Expired" : new Date(l.expires_at).toLocaleString("en-IN", { hour: "2-digit", minute: "2-digit", day: "numeric", month: "short" })}
                        </span>
                      </div>
        ...
        {tab === "shop" && (
          <div style={{ maxWidth: 560 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              ...
              {shopSaved && (
                <div style={{ fontSize: 13, color: "#27ae60", display: "flex", alignItems: "center", gap: 5 }}>
                  <Check size={13} /> Saved
                </div>
              )}
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div
                ...
                style={{ width: "100%", height: 150, border: "1.5px dashed #e8e8e8", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", overflow: "hidden", background: "#fafafa", position: "relative" }}
              >
                {bannerPreview ? (
                  <>
                  ...
                  <div style={{ textAlign: "center", color: "#bbb" }}>
                    ...
                    <div style={{ fontSize: 13 }}>Upload banner photo</div>
                  </div>
                )}
              </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <button
                type="button"
                onClick={() => setShowShopLocationPicker(true)}
                style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, border: `1.5px solid ${shopLat ? "#27ae60" : "#e8e8e8"}`, borderRadius: 8, padding: "10px 14px", fontSize: 14, cursor: "pointer", background: shopLat ? "#f0faf4" : "white", color: shopLat ? "#27ae60" : "#bbb", fontFamily: "inherit", marginBottom: 8 }}
              >
                <MapPin size={14} color={shopLat ? "#27ae60" : "#bbb"} />
                {shopLat ? "Location pinned ✓" : "Pin your shop on the map"}
              </button>
            ...
            <button
              ...
              disabled={savingShop}
              style={{ background: "#111", color: "white", border: "none", borderRadius: 10, padding: "13px 28px", fontSize: 14, fontWeight: 600, cursor: savingShop ? "not-allowed" : "pointer", opacity: savingShop ? 0.7 : 1, fontFamily: "inherit" }}
            >
        ...
        <div className="overlay" onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div className="modal">
            ...
            <form onSubmit={submitListing} style={{ padding: "24px 28px" }}>
              {formErr && (
                <div style={{ background: "#fff0f0", border: "1px solid #fdd", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#c00", marginBottom: 18 }}>{formErr}</div>
              )}
              {/* Image upload */}
              <div style={{ marginBottom: 20 }}>
                ...
                {imagePreview ? (
                  <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", height: 160 }}>
                    ...
                    <button type="button" onClick={() => { setImageFile(null); setImagePreview(null); }} style={{ position: "absolute", top: 8, right: 8, background: "white", border: "none", borderRadius: "50%", width: 28, height: 28, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <X size={14} color="#444" />
                    </button>
                  </div>
                ) : (
      ...
      {showLocationPicker && (
        <LocationPicker value={listingLocation} onChange={loc => { setListingLocation(loc); setShowLocationPicker(false); }} onClose={() => setShowLocationPicker(false)} />
      )}
      {showShopLocationPicker && (
        <LocationPicker
          value={shopLat ? { lat: shopLat, lng: shopLng } : null}
          onChange={loc => { setShopLat(loc.lat); setShopLng(loc.lng); setShopAddress(loc.address || shopAddress); setShowShopLocationPicker(false); }}
          onClose={() => setShowShopLocationPicker(false)}
        />
      )}
    </div>
  );
